...
function Admin({ onBack }) {
  ...
  const tabs = ['Visão geral', 'Rifas', 'Pedidos', 'Puxa número'];
  return <div className="admin"><aside><Logo /><div className="admin-user"><i>RF</i><span><b>Administrador</b><small>sessão autenticada</small></span></div>{tabs.map(t => <button className={tab === t ? 'active' : ''} onClick={() => setTab(t)} key={t}>{t === 'Puxa número' ? <Trophy /> : <span>◈</span>}{t}</button>)}<button className="exit" onClick={() => { sessionStorage.removeItem('rf_token'); onBack(); }}>← Sair</button></aside><section className="admin-main"><div className="admin-mobile-head"><Logo /><button onClick={() => { sessionStorage.removeItem('rf_token'); onBack(); }}>Sair</button></div><div className="admin-mobile-nav">{tabs.map(t => <button className={tab === t ? 'active' : ''} onClick={() => setTab(t)} key={t}>{t}</button>)}</div><div className="admin-top"><div><small>PAINEL ADMINISTRATIVO</small><h1>{tab}</h1></div><span>● Dados do servidor</span></div>
